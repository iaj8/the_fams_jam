...
public class GameCanvas {
  ...
  public void draw(Texture image, Color tint, float ox, float oy, 
    ...
    // Call the master drawing method
    holder.setRegion(image);
    draw(holder,tint,ox,oy,x,y,sx,sy);
  }
  ...
  public void draw(TextureRegion region, Color tint, float ox, float oy, 
    ...
    computeTransform(ox,oy,x,y,sx,sy);
    spriteBatch.setColor(tint);
    spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
    
    if (x <= 10.0f) {
    	computeTransform(ox,oy,x+width,y,sx,sy);
	    spriteBatch.setColor(tint);
	    spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
    } else if (x >= width-10.0f) {
    	computeTransform(ox,oy,x-width,y,sx,sy);
    ...
    if (y <= 10.0f) {
    	computeTransform(ox,oy,x,y+height,sx,sy);
	    spriteBatch.setColor(tint);
	    spriteBatch.draw(region,region.getRegionWidth(),region.getRegionHeight(),local);
    } else if (y >= height-10.0f) {
    	computeTransform(ox,oy,x,y-height,sx,sy);
   ...
   * @param sy  The y-axis scaling factor
   */
  private void computeTransform(float ox, float oy, float x, float y, float sx, float sy) {
    local.set(global);
    local.translate(x,y);
    local.scale(sx,sy);
    local.translate(-ox,-oy);
  ...
  public void drawMeText(String text, float x, float y, int align_width){  
	  displayFont.draw(spriteBatch, text, x,y, align_width, Align.center, true);
  }
