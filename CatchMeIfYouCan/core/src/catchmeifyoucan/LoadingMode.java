...
public class LoadingMode implements ModeController, InputProcessor, ControllerListener {
	...
	private static float PLAY_BUTTON_RATIO = 0.65f;
	...
	private void drawProgress(GameCanvas canvas) {	
		// In practice I would do this with affine transforms, but that would make
		// Exercise 4 harder.
		canvas.draw(statusBkgLeft, Color.WHITE, 0, 0, centerX-width/2, centerY,
					0, scale, scale);
		canvas.draw(statusBkgRight, Color.WHITE, 0, 0, centerX+width/2-scale*PROGRESS_CAP, centerY,
					0, scale, scale);
		canvas.draw(statusBkgMiddle, Color.WHITE, 0, 0, centerX-width/2+scale*PROGRESS_CAP, centerY,
				    0, (width-2*scale*PROGRESS_CAP)/((float)PROGRESS_MIDDLE), scale);

		canvas.draw(statusFrgLeft, Color.WHITE, 0, 0, centerX-width/2, centerY,
					0, scale, scale);
		if (progress > 0) {
			float span = progress*(width-2*scale*PROGRESS_CAP)/2.0f;
			canvas.draw(statusFrgRight, Color.WHITE, 0, 0, centerX-width/2+scale*PROGRESS_CAP+span, centerY,
						0, scale, scale);
			canvas.draw(statusFrgMiddle, Color.WHITE, 0, 0, centerX-width/2+scale*PROGRESS_CAP, centerY,
					    0, span/((float)PROGRESS_MIDDLE), scale);
		} else {
			canvas.draw(statusFrgRight,  Color.WHITE, 0, 0, centerX-width/2+scale*PROGRESS_CAP, centerY,
						0, scale, scale);
		}
	}
